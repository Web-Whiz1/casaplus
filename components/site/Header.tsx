...
export function Header() {
  ...
  return (
    <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-700 ease-out will-change-transform ${solid ? 'bg-canvas/95 backdrop-blur-md shadow-sm border-b border-line' : 'bg-transparent'}`}>
      <div className="container-lux flex items-center justify-between h-20 md:h-24">
        ...
        <button className="lg:hidden transition-transform duration-300 ease-out active:scale-90" onClick={() => setOpen(true)} aria-label="Menu">
          <Menu className={`transition-all duration-700 ease-out ${solid ? 'text-ink' : 'text-white'}`} size={26} />
        </button>
      </div>

      <div
        className={`fixed inset-0 z-[60] bg-canvas flex flex-col transition-all duration-500 ease-out ${
      ...
      >
        <div className="container-lux flex items-center justify-between h-20">
          ...
          <button onClick={() => setOpen(false)} aria-label="Close" className="transition-transform duration-300 ease-out active:scale-90"><X size={26} /></button>
        </div>
        <div className="flex-1 flex flex-col justify-center container-lux gap-6">
          {nav.map((n, i) => (
            <Link
              ...
              className="font-display text-4xl text-ink hover:text-gold transition-all duration-300 ease-out translate-x-[-20px] opacity-0"
              style={open ? { transitionDelay: `${i * 80}ms`, transform: 'translateX(0)', opacity: 1 } : {}}
            >
              {t(n.k)}
            </Link>
          ))}
